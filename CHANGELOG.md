...
- **wasm**: `ldap` capability with three host functions, `host_ldap_bind`, `host_ldap_search` and `host_ldap_read_result`, backed by a native LDAP client (`ldap3` with rustls on aws-lc-rs, no new TLS stack). Credential binds run on a fresh connection each time; service-account search connections are pooled per URL and bind identity (bounded, liveness-checked). Directory egress goes through the plugin SSRF guard (`BARBACANE_ALLOW_INTERNAL_EGRESS`), plaintext connections are pinned to the vetted address, StartTLS fails closed, and searches are capped at 1000 entries / 1 MiB. Results carry a stable `code` (`invalid_credentials`, `connection_failed`, `timeout`, ...) so an auth plugin can map rejected credentials to 401 and directory failures to 502. Decision record: ADR-0032, which supersedes ADR-0028.
