...
- **compiler**: a plugin configuration value given as a runtime reference (`env://`, `file://`, `vault://`, `aws-sm://`, `k8s://`) is no longer refused by `E1023` for not looking like the value it stands for. The value is resolved when the gateway loads the artifact, so its content cannot be checked at compile time. A `ws-upstream` dispatch reading its `url` from `env://` failed the compile against the plugin's `^wss?://` pattern; an `http-upstream` one passed only because `env://NAME` happens to be a valid URI. A reference where the schema expects a number or an object is still refused, since it resolves to a string.
...
- **plugins/ldap-auth**: LDAP / Active Directory authentication middleware. `Authorization: Basic` credentials are verified by locating the user entry with a service-account search (`user_filter`, `{username}` escaped per RFC 4515) and binding as that entry; groups come from `memberOf` or a configurable group search and are emitted as `x-auth-consumer-groups` for `acl`. Sets `x-auth-consumer`, `x-auth-user`, `x-auth-dn`; strips `Authorization`; answers 401 with a Basic challenge (identical for unknown user and wrong password), 403 when `required_groups` is not met, 503 when the directory is unavailable. Verified and rejected credentials are cached for `cache_ttl_seconds`; empty passwords are rejected without a bind; a password is sent over plaintext `ldap://` only with `allow_plaintext: true`. Integration-tested against a glauth directory in CI.
...
- **data plane**: a request carries only the headers its operation admits. The baseline every operation accepts (message framing and negotiation, CORS, tracing, the WebSocket handshake, and the proxy chain as it arrives), plus the `in: header` parameters the operation declares, `cookie` when it declares an `in: cookie` parameter, the credential header named by its security scheme, and the headers a plugin in its chain reads. Everything else is dropped before any plugin or upstream sees it, so the spec describes what an upstream receives rather than only what a client may send. Decision record: ADR-0033.
  ...
  > **Upgrade note (breaking):** **a header not described by the spec no longer reaches your upstream.** Declare it as an `in: header` parameter on the operation, on the path item, or through `components/parameters`, for it to travel. `authorization` is not in the baseline: it travels because the operation's `security` requirement names a scheme carrying it, and for no other reason. `cookie` travels only when the operation declares an `in: cookie` parameter. Dropped headers are counted by `barbacane_request_headers_dropped_total`, and `serve --dev` logs each one by name, so a first run against an existing spec shows exactly what to declare.

- **compiler**: `E1057` refuses an operation that runs an authentication middleware without naming the security scheme carrying the credential. A plugin's `plugin.toml` states its family in a new `category` field, and `authentication` is the one the compiler acts on, so a third-party auth plugin behaves the same way without the compiler knowing its name. `category` also carries the grouping the middleware guide documents.

  > **Upgrade note (breaking):** **an operation running `basic-auth`, `jwt-auth`, `apikey-auth`, `oauth2-auth`, `oidc-auth` or `ldap-auth` must now declare a `security` requirement** naming a scheme defined under `components.securitySchemes`, on the operation or at the root. Attaching the middleware through `x-barbacane-middlewares` alone described the operation as anonymous while requiring a credential, so nothing admitted the credential header and generated documentation and clients were wrong about it. The compile error names the operation and the plugin. An operation deliberately outside the chain keeps `x-barbacane-middlewares: []` and takes no requirement.

  > **Note for plugin authors:** a plugin's manifest and config schema are embedded in its `.wasm` at build time, so rebuild the plugin after editing either for the compiler to see the change.

- **plugins**: `config-schema.json` is embedded in the `.wasm` by the SDK macros, as `plugin.toml` already was. The schema carries the annotations naming the request headers a plugin reads, and it was only ever read from a file beside the binary, which neither the container image nor the release download ships. Compiled from those, an operation lost every header its plugin's configuration names: a `rate-limit` partitioned on `header:x-client-id` admitted no such header, so the gateway limited every caller in one bucket instead of per client, silently. The schema now travels inside the binary, so the compiler reads it from a container holding only the `.wasm`, from a release download, and from a URL-sourced plugin, which could never supply one before. Plaintext-secret detection (`E1070`) reaches all three for the same reason, and the control plane reads both the schema and the category out of the binary its registry stores.
...
- **plugins (auth)**: `basic-auth`, `apikey-auth`, `jwt-auth`, `oauth2-auth`, `oidc-auth` and `ldap-auth` now publish the verified identity in the request context as `auth.sub` and `auth.groups` (when the identity has groups), alongside the `x-auth-*` headers, so `context:auth.sub` and `context:auth.groups` work as documented for `rate-limit`, `request-transformer` and `opa-authz` input mappings. The SDK gains `barbacane_plugin_sdk::context` (`get`, `set`, the `AUTH_SUB` / `AUTH_GROUPS` keys, and a thread-local store on native targets for tests).
...
- **wasm**: LDAP `ldaps://` and StartTLS connections are opened to the address vetted by the SSRF guard, closing the DNS-rebinding window between resolution and connect that plaintext connections already had closed. The socket is handed to `ldap3` with the URL, so SNI and certificate validation still use the hostname.
- **wasm**: NATS refuses the servers a broker advertises in `INFO.connect_urls`. They entered the reconnect pool without passing the SSRF guard, so a broker could steer a reconnect at an internal address. A `tls://` server is still reached by hostname: `async_nats` derives the TLS server name from the address it is given, and an IP literal suppresses SNI, so pinning the address would break a broker that selects its certificate by SNI.
...
- **WAF**: the `@detectSQLi` and `@detectXSS` operators are implemented, so OWASP CRS rules 941100, 941101, 942100 and 942101 (the libinjection classifiers) are enforced rather than skipped, and stock CRS v4.9.0 compiles in full. They run on the [`libinjectionrs`](https://github.com/barbacane-dev/libinjectionrs) port, which is differential-tested against the C library through an FFI harness over its ~163,000-input corpus and matches it on every input, both verdicts and fingerprints, with any divergence failing CI. `@detectSQLi` captures the fingerprint into `TX:0`, as ModSecurity does. ADR-0031 is accepted.
...
- **plugin**: `ai-proxy` no longer returns `404 Not Found` when the operation is bound to a path other than `/v1/chat/completions`. The path-based dispatch added in PR-1 was too strict — operators are free to bind `ai-proxy` to any operation path, and the dispatcher routes Chat Completions requests through unchanged.
...
[0.6.1]: https://github.com/barbacane-dev/Barbacane/compare/v0.6.0...v0.6.1
[0.6.0]: https://github.com/barbacane-dev/Barbacane/compare/v0.5.2...v0.6.0
[0.5.2]: https://github.com/barbacane-dev/Barbacane/compare/v0.5.1...v0.5.2
[0.5.1]: https://github.com/barbacane-dev/Barbacane/compare/v0.5.0...v0.5.1
[0.5.0]: https://github.com/barbacane-dev/Barbacane/compare/v0.4.1...v0.5.0
