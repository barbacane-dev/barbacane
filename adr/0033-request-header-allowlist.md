...
- A plugin referenced by URL rather than path has no `config-schema.json` available to the compiler, so set 4 contributes nothing for it and the headers it reads must be declared as parameters. This is the same gap that already limits secret-field detection. The control plane compiles from a registry holding neither the schema nor the category, so the same applies there and the authentication check does not run. Each such plugin is reported as `E1071`.
