...
1. **Baseline**, fixed and documented:
   ...
   - Browser request metadata, which a browser sends unasked and which describes the request rather than the caller: `referer`, `sec-fetch-site`, `sec-fetch-mode`, `sec-fetch-dest`, `sec-fetch-user`, `sec-ch-ua`, `sec-ch-ua-mobile`, `sec-ch-ua-platform`, `dnt`, `sec-gpc`, `priority`. `user-agent`, `accept-language` and `origin` above are the same class. The fetch-metadata set matters most: an upstream uses it to reject cross-site requests, so dropping it silently removes a defence the upstream believes it has.
...
Pairing a plugin with a scheme it does not implement, such as `jwt-auth` with an `apiKey`, is `E1032`. An authentication plugin lists the scheme types it reads in `plugin.toml` as `implements`, using the tokens `apiKey`, `http:<scheme>`, `oauth2`, `openIdConnect` and `mutualTLS`, and an operation whose requirement resolves to none of them is refused. A plugin declaring no list is exempt, so the mechanism stays opt-in for a third-party plugin exactly as `category` does.
...
The same model is what `E1032` (a referenced scheme has no matching auth middleware) and the scheme-defined-but-never-referenced check need. `E1032` is built on it, together with `E1033`, the warning for an operation whose requirement nothing in its chain verifies. `E1040` is already in use for `UndeclaredPlugin`, so the remaining check takes a free code.
