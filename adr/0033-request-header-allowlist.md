...
3. **Security schemes**: the headers named by the security schemes the operation's `security` requirement applies. AsyncAPI declares the requirement on the server rather than the operation, so an operation there inherits it from the servers its channel is reached through: each server's schemes are alternatives, a channel narrowed with `servers:` inherits only from those, and one that is not is reachable on all of them. An `apiKey` scheme with `in: header` contributes its `name`; an `apiKey` scheme with `in: cookie` contributes `cookie`; `http` schemes (`basic`, `bearer`, and any other scheme) and `oauth2` / `openIdConnect` contribute `authorization`. `Authorization` is therefore never in the baseline: it is forwarded because the spec says the operation is authenticated.
