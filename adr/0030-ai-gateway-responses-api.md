...
- **Generates a synthetic `id`** in the response (format `resp_<uuid-v4>`). Non-deterministic — matches the OpenAI Responses API contract, where the `id` is opaque and per-request. Clients that read the ID only as a tracking handle are unaffected; clients that try to reuse it fail explicitly at the next point.
