...
- **SSRF guard.** The directory host is resolved through the same `resolve_permitted_addrs` check as plugin HTTP calls, brokers and WebSocket upstreams, honouring `BARBACANE_ALLOW_INTERNAL_EGRESS`. Every connection, plaintext or TLS, is opened to a vetted address; the socket is handed to `ldap3` together with the original URL, so `ldaps://` and StartTLS still derive SNI and certificate validation from the hostname. NATS cannot do the same: `async_nats` takes no pre-connected stream and derives the TLS server name from the address it is given, so a `tls://` broker keeps its hostname and its rebinding window.
