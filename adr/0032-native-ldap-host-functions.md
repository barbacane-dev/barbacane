...
- **Search connections are pooled** and bound as the service account named in the request. The cache key is the URL, the bind DN and a fingerprint of the password, so rotated credentials do not reuse a stale bind. The pool holds at most 256 connections; a closed connection is evicted and re-established on next use; a failed search evicts its connection.
...
- **Bounds.** Connect timeout 5 s; per-operation timeout from the request, clamped to 30 s, default 10 s; at most 1000 entries per search; at most 1 MiB of serialized entries. Binary attributes are omitted from results.
