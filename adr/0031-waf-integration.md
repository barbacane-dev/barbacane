...
- ~~Response body inspection (CRS phase 4) interacts with streaming ([ADR-0023](0023-wasm-plugin-streaming.md)): a streamed response is already on the wire before phase 4 could block it. Detection-only for streamed responses, or refuse to combine the two?~~ Resolved: phases 3, 4 and 5 run on the response. Phase 3 (headers) runs on every path, including streaming. Phase 4 (body) runs on a buffered body at or under `max_response_body`; a streamed or oversized body has its headers inspected and its body skipped, counted in `barbacane_waf_response_body_skipped_total`. A WebSocket upgrade has no response phases.
