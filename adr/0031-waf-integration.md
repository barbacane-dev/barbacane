...
1. **Per-request rule compilation disappears.** The artifact carries the rule set already parsed, validated and canonicalised; the data plane builds the regex programs and Aho-Corasick automata once at startup and reuses them for every request. Per-request cost is evaluation only.
   ...
   316 ms per process is acceptable for a cold start and is not acceptable to pay on every hot reload, so artifact reload needs to either rebuild off the request path or keep the previous rule set live until the new one is ready. Worth settling when the pipeline stage is designed rather than after.
