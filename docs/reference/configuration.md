...
| `BARBACANE_ALLOW_INTERNAL_EGRESS` | Data plane | `false` | Set to `1`/`true` to disable the plugin SSRF guard and allow plugin egress (HTTP calls, Kafka/NATS broker connections, **and** WebSocket upstreams) to internal/loopback/link-local/cloud-metadata addresses. Every guarded client connects to the address vetted by the guard rather than resolving again (DNS-rebinding safe); TLS still validates the server certificate against the URL hostname. Leave off unless you have legitimate internal upstreams or brokers. |
