...
"#,
    ...
    async fn test_metrics_endpoint_returns_prometheus_format() {
        ...
        // Get the metrics endpoint (moved to admin port in ADR-0022)
    ...
    async fn test_metrics_records_request_counts() {
        ...
        // Get metrics (moved to admin port in ADR-0022)
        let resp = gateway.admin_get("/metrics").await.unwrap();
        let body = resp.text().await.unwrap();
...
"#,
    ...
    async fn test_response_transformer_status_mapping() {
        let gateway = TestGateway::from_spec("../../tests/fixtures/response-transformer.yaml")
            .await
            .expect("failed to start gateway");
